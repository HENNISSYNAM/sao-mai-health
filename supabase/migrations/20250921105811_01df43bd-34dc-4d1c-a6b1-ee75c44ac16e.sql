-- Create disease reference table if not exists
...
-- Insert disease codes
INSERT INTO ref_diseases (code, name_vi, name_en, category) VALUES
('dengue', 'Sốt xuất huyết', 'Dengue Fever', 'vector_borne'),
('covid19', 'COVID-19', 'COVID-19', 'respiratory'),
('hfmd', 'Bệnh tay chân miệng', 'Hand Foot Mouth Disease', 'viral'),
('ari', 'Nhiễm khuẩn hô hấp', 'Acute Respiratory Infection', 'respiratory'),
('malaria', 'Sốt rét', 'Malaria', 'vector_borne'),
('h1n1', 'Cúm A/H1N1', 'Influenza A/H1N1', 'respiratory'),
('tcm', 'Tay chân miệng', 'Hand Foot Mouth', 'viral')
...
-- Recent cases for real-time feel
...
) WHERE patient_hash = 'DEN001';

UPDATE case_events SET patient_id = (
  SELECT id FROM patients WHERE full_name = 'Trần Thị Bình' LIMIT 1  
) WHERE patient_hash = 'COV001';

UPDATE case_events SET patient_id = (
  SELECT id FROM patients WHERE full_name = 'Lê Văn Cường' LIMIT 1
) WHERE patient_hash = 'HFM001';