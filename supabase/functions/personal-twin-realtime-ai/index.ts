...
Trả về JSON theo định dạng đã quy định.`;
        ...
        try {
          const aiResponse = await callLovableAI(TWIN_AI_SYSTEM_PROMPT, userPrompt);
          ...
          if (error instanceof Error) {
            if (error.message === 'RATE_LIMIT_EXCEEDED') {
              return new Response(
                JSON.stringify({ success: false, error: 'Đã vượt giới hạn yêu cầu. Vui lòng thử lại sau.' }),
                { status: 429, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
              );
            }
            if (error.message === 'PAYMENT_REQUIRED') {
              return new Response(
                JSON.stringify({ success: false, error: 'Cần nạp thêm credits. Vui lòng liên hệ quản trị.' }),
...
Trả về JSON theo định dạng đánh giá rủi ro.`;
        ...
        const aiResponse = await callLovableAI(RISK_ASSESSMENT_PROMPT, userPrompt);
        const parsed = parseAIResponse(aiResponse);
...
Phân tích nhanh và đưa ra insights quan trọng nhất. Ưu tiên tính kịp thời.`;
        ...
        const streamResponse = await callLovableAI(TWIN_AI_SYSTEM_PROMPT, userPrompt, true);
