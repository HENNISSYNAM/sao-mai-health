...
import { useSensorDataSync } from './useSensorDataSync';
...
export function useStrokeRiskEngine({ onRiskChange }: UseStrokeRiskEngineProps = {}) {
  ...
  const sensorSync = useSensorDataSync({
    sensorState: deviceSensors,
    environment,
    enabled: deviceSensors.isAnyActive,
  });
  ...
  const fetchEnvironment = useCallback(async (lat: number, lon: number, forceRefresh = false) => {
    ...
    if (!forceRefresh && shouldUseCachedEnv(lat, lon)) {
      ...
      console.log('Using cached environment data');
      return;
    }
    ...
    setEnvLoading(true);
    console.log('Fetching fresh environment data...');
  ...
  const startGPSTracking = useCallback(() => {
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      async (position) => {
        ...
        // IMMEDIATELY fetch environment data on GPS update
  ...
  const calculateRisk = useCallback((): RiskAssessment => {
    ...
    // Sensor-based risk factors (0-20 points)
    if (deviceSensors.isAnyActive) {
      const { health } = deviceSensors;
      
      // Tremor detection (0-10 points)
      if (health.tremorDetected) {
        const tremorPoints = Math.round(health.tremorIntensity * 10);
        score += tremorPoints;
        factors.push('Phát hiện run tay');
        doRecommendations.push('Kiểm tra sức khỏe thần kinh');
      }
      
      // Balance issues (0-8 points)
      if (health.balanceIssue) {
        score += 8;
        factors.push('Mất thăng bằng');
        doRecommendations.push('Ngồi nghỉ, tránh di chuyển');
      } else if (health.balanceScore < 60) {
        score += 4;
      }
      
      // Sedentary lifestyle (0-5 points)
      if (health.activityLevel === 'sedentary' && health.steps < 500) {
        score += 5;
        factors.push('Ít vận động');
        doRecommendations.push('Đi bộ ít nhất 30 phút mỗi ngày');
      }
      
      // Fall detection (immediate high risk)
      if (health.fallDetected) {
        score += 15;
        factors.push('Phát hiện té ngã');
        doRecommendations.push('Kiểm tra chấn thương ngay');
      }
    }
  ...
  }, [userData.ageGroup, userData.outdoorMinutes, environment, barometer.pressureChange1h, deviceSensors.isAnyActive, deviceSensors.health]);
  ...
  }, [userData.ageGroup, userData.outdoorMinutes, environment.aqi, environment.pm25, environment.temperature, environment.humidity, barometer.pressureChange1h, deviceSensors.health.tremorDetected, deviceSensors.health.balanceIssue, deviceSensors.health.fallDetected, deviceSensors.health.activityLevel]);
  ...
  const startMonitoring = useCallback(async () => {
    ...
    // PRIORITY 1: Get GPS and Environment data FAST
    const getInstantGPS = (): Promise<{ lat: number; lon: number; accuracy: number } | null> => {
      return new Promise((resolve) => {
        let resolved = false;
        
        // First try: cached/low accuracy for INSTANT result
        navigator.geolocation.getCurrentPosition(
          (pos) => {
            if (!resolved) {
              resolved = true;
          ...
          { enableHighAccuracy: false, timeout: 1500, maximumAge: 600000 } // 10 min cache, fast
        );
    ...
    // Get instant GPS and fetch environment IMMEDIATELY
    const instantGps = await getInstantGPS();
    if (instantGps) {
      console.log('Got instant GPS, fetching environment immediately...');
      
      // Update GPS in state
      setUserData(prev => ({
        ...prev,
        gps: { lat: instantGps.lat, lon: instantGps.lon },
        gpsAccuracy: instantGps.accuracy,
        timestamp: Date.now()
      }));
      
      // Fetch environment data IMMEDIATELY (don't wait)
      fetchEnvironment(instantGps.lat, instantGps.lon, true);
    }
  ...
  const stopMonitoring = useCallback(() => {
    ...
    deviceSensors.stopAll();
  }, [stopGPSTracking, barometer, deviceSensors]);
