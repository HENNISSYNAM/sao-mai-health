...
export default function LabImport() {
  ...
  const processImport = async () => {
    ...
    try {
      ...
      const loincMap: Record<string, { name: string; category: string; unit?: string }> = {
        '33747-0': { name: 'Dengue IgM', category: 'blood' },
        '31996-4': { name: 'Dengue IgG', category: 'blood' },
        '25835-0': { name: 'Dengue RNA', category: 'blood' },
        '6690-2':  { name: 'Bạch cầu (WBC)', category: 'blood', unit: 'K/uL' },
        '777-3':   { name: 'Tiểu cầu (PLT)', category: 'blood', unit: 'K/uL' },
        '718-7':   { name: 'Hemoglobin', category: 'blood', unit: 'g/dL' },
      }
      ...
      for (const row of previewData) {
        for (const col of mappedColumns) {
          if (!col.mappedTo || !loincMap[col.mappedTo]) continue
          const meta = loincMap[col.mappedTo]
      ...
      if (rows.length > 0) {
        const { error } = await supabase.from('biovault_metrics').insert(rows)
        if (error) throw error
      }

      setProgress(100)
      toast.success(`Đã import thành công ${rows.length} chỉ số xét nghiệm`)
