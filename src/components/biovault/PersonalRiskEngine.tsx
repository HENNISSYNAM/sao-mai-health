...
export const PersonalRiskEngine: React.FC<PersonalRiskEngineProps> = ({ 
  ...
  const historicalData = Array.from({ length: 24 }, (_, i) => ({
    hour: `${i}:00`,
    risk: Math.max(20, Math.min(90, 50 + Math.sin(i / 4) * 30 + Math.random() * 10)),
    pressure: 1013 - Math.sin(i / 6) * 5 + Math.random() * 2
  }));
    ...
    <Card className="border-2 border-primary/20 bg-card/95 backdrop-blur">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          {[
            { icon: Cloud, label: t('biovault.env.pressure', 'Áp suất'), value: `${environment.pressure} hPa`, warning: environment.pressure < 1010 },
            { icon: Droplets, label: t('biovault.env.humidity', 'Độ ẩm'), value: `${environment.humidity}%`, warning: environment.humidity > 85 },
            { icon: Thermometer, label: t('biovault.env.temp', 'Nhiệt độ'), value: `${environment.temperature}°C`, warning: environment.temperature > 35 },
            { icon: Wind, label: 'AQI', value: environment.aqi.toString(), warning: environment.aqi > 75 },
            { icon: Zap, label: 'UV', value: environment.uvIndex.toString(), warning: environment.uvIndex > 7 },
