...
const FullScreenMapInner: React.FC<FullScreenMapProps> = ({
  ...
  const updateMapNow = useCallback((offsetDelta: {
    lat: number;
    lon: number;
  } | null, zoomDelta: number) => {
    if (offsetDelta) {
      setMapOffset(prev => ({
  ...
  useEffect(() => {
    ...
    switch (action) {
      ...
      case 'toggle_layer':
        ...
        setMapKey(prev => prev + 1);
        feedbackText = showAQILayer ? '🌬️' : '💨 PM2.5';
        break;
      case 'reset_view':
        setMapOffset({
          lat: 0,
  ...
  const getAQIInfo = (aqi: number | null) => {
    ...
    if (aqi <= 50) return {
      ...
      label: t('epiIntel.kpi.aqiGood'),
      textColor: 'text-white'
    };
    if (aqi <= 100) return {
      color: 'bg-yellow-500',
      label: t('epiIntel.kpi.aqiModerate'),
      textColor: 'text-black'
    };
    if (aqi <= 150) return {
      color: 'bg-orange-500',
      label: t('epiIntel.kpi.aqiUnhealthy'),
      textColor: 'text-white'
    };
    if (aqi <= 200) return {
      color: 'bg-red-500',
      label: t('epiIntel.kpi.aqiBad'),
      textColor: 'text-white'
    };
    return {
      color: 'bg-purple-600',
      label: t('epiIntel.kpi.aqiHazardous'),
      textColor: 'text-white'
    };
  };
  ...
  const formatOutdoorTime = (minutes: number) => {
    if (minutes < 60) return `${minutes} ${t('epiIntel.outdoor.minutes', { count: minutes }).split(' ').pop()}`;
  ...
  return <div ref={mapContainerRef} className={cn("absolute inset-0 transition-all duration-700 overflow-hidden", isBlurred && "scale-[1.02] brightness-[0.3] blur-sm", className)}>
      {/* Windy Map - key forces complete reload when switching layers */}
      <iframe key={`windy-map-${mapKey}`} src={windyUrl} className="absolute inset-0 w-full h-full border-0" style={{
      minHeight: '100vh',
      minWidth: '100vw'
    }} allow="geolocation" title={showAQILayer ? "Windy AQI Map" : "Windy Weather Map"} />
      
      {/* Hand Control Layer - pluggable input layer */}
      {!isBlurred && <MapHandControlLayer containerRef={mapContainerRef} onPan={handleHandPan} onZoom={handleHandZoom} onContextMenu={handleHandContextMenu} />}
      ...
      {gestureActionFeedback && <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 animate-fade-in pointer-events-none">
      ...
      {!isBlurred && showDataPanel && <div className="absolute top-16 left-4 z-20 animate-fade-in">
          <div className="flex flex-col gap-1.5 my-[300px]">
            {/* Close button */}
            <button onClick={() => setShowDataPanel(false)} className="w-9 h-9 flex items-center justify-center bg-black/40 backdrop-blur-md rounded-full border border-white/10 hover:bg-black/60 transition-all active:scale-95 self-start">
            ...
            <button onClick={toggleAQILayer} className={cn("flex items-center gap-2 px-3 py-2 rounded-full backdrop-blur-md border border-white/10 transition-all active:scale-95", showAQILayer ? aqiInfo.color : "bg-black/40 hover:bg-black/60")}>
              ...
              {showAQILayer ? <Eye className="h-3 w-3 text-white/70" /> : <EyeOff className="h-3 w-3 text-white/50" />}
            </button>
            ...
            {/* Location Type Pill */}
            <div className={cn("flex items-center gap-2 px-3 py-2 rounded-full backdrop-blur-md border border-white/10", isOutdoor ? outdoorMinutes >= safeOutdoorMinutes ? 'bg-red-500/80' : 'bg-blue-500/80' : 'bg-black/40')}>
              {isOutdoor ? <TreePine className="h-4 w-4 text-white" /> : <Home className="h-4 w-4 text-white" />}
              <span className="text-sm font-semibold text-white">
                {isOutdoor ? outdoorMinutes > 0 ? formatOutdoorTime(outdoorMinutes) : t('epiIntel.map.outdoor') : t('epiIntel.map.indoor')}
              </span>
            </div>

            {/* Statistics button */}
            {showStatisticsButton && onViewStatistics && <button onClick={onViewStatistics} className="flex items-center gap-2 px-3 py-2 bg-slate-700/80 hover:bg-slate-600/80 backdrop-blur-md rounded-full border border-white/10 transition-all active:scale-95">
                <BarChart3 className="h-4 w-4 text-white" />
                <span className="text-sm font-semibold text-white">{t('epiIntel.statistics')}</span>
              </button>}
          </div>
        </div>}

      {/* GPS Position Card - Fixed bottom center with AI geocoded address */}
      {gps && !isBlurred && <div className="absolute bottom-24 left-1/2 -translate-x-1/2 z-20 animate-fade-in max-w-[90vw]">
          <div className="flex items-center gap-3 px-4 py-2.5 bg-card/90 backdrop-blur-xl rounded-2xl border border-border/30 shadow-xl">
            <div className="flex items-center justify-center w-8 h-8 bg-blue-500 rounded-full shrink-0">
              {isGeocoding ? <Loader2 className="h-4 w-4 text-white animate-spin" /> : <MapPin className="h-4 w-4 text-white" />}
            </div>
            <div className="min-w-0">
              <div className="text-xs text-muted-foreground">{t('epiIntel.yourLocation')}</div>
              {geocodedLocation ? <>
                  <div className="text-sm font-medium text-foreground truncate max-w-[250px]">
                    {geocodedLocation.address}
                  </div>
                  <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                    {geocodedLocation.district && <span>{geocodedLocation.district}</span>}
                    {geocodedLocation.city && <span className="text-blue-500 font-medium">{geocodedLocation.city}</span>}
                    {geocodedLocation.area_type && geocodedLocation.area_type !== 'Không xác định' && <span className="px-1.5 py-0.5 bg-emerald-500/20 text-emerald-600 rounded">
              ...
              {gpsAccuracy && <div className="text-[10px] text-muted-foreground">±{gpsAccuracy.toFixed(0)}m</div>}
            </div>
          </div>
        </div>}

      {/* Tracking status badge - top right (next to home button) - Enhanced with weather data */}
      {!isBlurred && isTracking && <div className="absolute top-3 right-16 z-20 animate-fade-in">
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-emerald-500 backdrop-blur-xl rounded-full shadow-xl mx-[356px]">
            ...
            {/* Pressure */}
            {displayPressure && <div className="flex items-center gap-0.5">
      ...
      {!isBlurred && showAQILayer && <div className="absolute top-16 left-4 z-20 animate-fade-in">
          <div className={cn("px-3 py-1.5 rounded-lg shadow-lg border", aqiInfo.color, "border-white/20")}>
            <div className="flex items-center gap-2">
              <Wind className="h-4 w-4 text-white" />
              <span className="text-xs font-semibold text-white">{t('epiIntel.map.pollutionMap')}</span>
            </div>
            <p className="text-[10px] text-white/80 mt-0.5">{t('epiIntel.map.clickAqiToClose')}</p>
          </div>
        </div>}
...
const FullScreenMap = memo(FullScreenMapInner, (prevProps, nextProps) => {
  ...
  const gpsChanged = prevProps.gps?.lat !== nextProps.gps?.lat || prevProps.gps?.lon !== nextProps.gps?.lon;
  const envChanged = prevProps.environment.aqi !== nextProps.environment.aqi || prevProps.environment.temperature !== nextProps.environment.temperature || prevProps.environment.humidity !== nextProps.environment.humidity || prevProps.environment.pressure !== nextProps.environment.pressure;
  const trackingChanged = prevProps.isTracking !== nextProps.isTracking;
  const blurChanged = prevProps.isBlurred !== nextProps.isBlurred;
  const outdoorChanged = prevProps.isOutdoor !== nextProps.isOutdoor || prevProps.outdoorMinutes !== nextProps.outdoorMinutes;
