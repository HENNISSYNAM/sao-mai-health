...
export function LivingAIChart() {
  ...
  const chartData = useMemo(() => {
    ...
    // Filter out unknown/unmapped diseases
    const validDiseases = prioritizedDiseases.filter(d => {
      const name = getDiseaseName(d.disease, language);
      return name !== 'Không xác định' && name !== 'Unknown';
    });
    ...
    for (let i = -7; i <= 14; i++) {
      ...
      validDiseases.slice(0, 4).forEach(disease => {
        if (i <= 0) {
          // Past data: gentle random walk around currentCases
          const variance = 1 + (Math.random() - 0.5) * 0.1; // ±5% instead of ±10%
          entry[disease.disease] = Math.round(disease.currentCases * variance);
        } else if (i <= 7) {
          // Smooth interpolation to 7d prediction
          const progress = i / 7;
          const diff = disease.predictedCases7d - disease.currentCases;
          entry[disease.disease] = Math.round(disease.currentCases + diff * progress);
        } else {
          // Smooth interpolation to 14d prediction
          const progress = (i - 7) / 7;
          const diff = disease.predictedCases14d - disease.predictedCases7d;
          entry[disease.disease] = Math.round(disease.predictedCases7d + diff * progress);
        }
      });
        ...
        {chartData.length > 0 ? (
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
              <defs>
                {prioritizedDiseases
                  .filter(d => { const n = getDiseaseName(d.disease, language); return n !== 'Không xác định' && n !== 'Unknown'; })
                  .slice(0, 4).map((disease) => {
                  const color = getDiseaseColor(disease.disease);
                  return (
